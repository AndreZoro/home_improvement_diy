...
def derive_gadget_stand_vals(dev_w=80, dev_h=140, dev_t=12, slant=20, laptop_slot=False, laptop_thickness=5):
    ...
    if laptop_slot:
        ...
        l_slot_add_length *= 1 + (max(slant -20, 0) / 20) * 0.15
...
def build_stand(
    dev_w=80, dev_h=140, dev_t=12, slant=20, laptop_slot=True, laptop_thickness=5
):
    alpha_rad, beta_rad, xr, xrr, yrr, t_ref, fl, xl, l_slot_len = derive_gadget_stand_vals(
    ...
    outer_chmf_len = min(t_ref / 4.0, laptop_thickness / 4.0,
                         dev_t / 4.0, sy2/6.0, y4/6.0)
    try:
