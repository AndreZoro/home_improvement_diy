...
def create_build123d_handle_v02(
    ...
    if (mid_width + 4*handle_champfer_l) < (b_width * 1.2):
        # TODO: This recaclulation is a bit wrong
        h_width = b_width * 1.2 + 6 * handle_champfer_l + b_height * abs(slant_ang) / 45 * 1.5
    ...
    if h_height < b_height * 1.2:
        ...
        handle_champfer_l *= 0.5
        print(f"Handle height was too small for the selected screw type. Handle height gets automatically increased to {h_width}mm.")
        h_delta_x, mid_width, h_pts = get_handle_dims(h_height, h_width, slant_ang)
