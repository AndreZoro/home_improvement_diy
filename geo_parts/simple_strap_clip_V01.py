...
def build_build123d_strap_clip(width=20, thickness=3, height=22, layer_width=0.4, n_shells=3,sid='none'):
    msgs = []
    ...
    if shell_thickness <= 0.1:
        shell_thickness = 0.2
        msgs.append(f"With current settings, the shell thickness is to low. Automatically increased to :{shell_thickness:.3f}mm. Check you shell count and layer width.")

    if width < thickness* 1.5:
        width = thickness * 1.5
        msgs.append(f"Width automatically increased to {width:.3f}mm. Check you chosen width, it is a bit narrow for the selected strap thickness.")


    # Inner lines
    l1 = Line((-(width-thickness) / 2, thickness / 2),
    ...
    l3 = Line(((width-thickness) / 2, -thickness / 2),
              (-(width-thickness) / 2, -thickness / 2))
    l4 = ThreePointArc(
        (-(width-thickness) / 2, -thickness / 2),
        (-width / 2, 0),
    ...
    # Outer lines
    ll1 = Line((-(width-thickness) / 2, thickness / 2 + shell_thickness),
    ...
    ll3 = Line(((width-thickness) / 2, -thickness / 2 - shell_thickness),
               (-(width-thickness) / 2, -thickness / 2 - shell_thickness))
    ll4 = ThreePointArc(
        (-(width-thickness) / 2, -thickness / 2 - shell_thickness),
        ((-width / 2) - shell_thickness, 0),
    ...
    try:
        ...
        msgs.append("Was not able to round the corners in the slot gap. Maybe you are using strange dimensions?")
    ...
    if n_shells >= 3:
        try:
            ...
            msgs.append("Was not able to champfer the outer edges. Maybe you are using strange dimensions?")
...
def build_strap_clip(
    ...
    pyvista_part, messages, part_file_name = build_build123d_strap_clip(width, thickness, height, layer_width, n_shells,sid)
